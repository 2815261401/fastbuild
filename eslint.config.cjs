const eslint = require('@eslint/js'); /** 基础 */
const tseslint = require('typescript-eslint'); /** typescript */
const pluginPrettier = require('eslint-plugin-prettier/recommended'); /** prettier */
const configPrettier = require('eslint-config-prettier'); /** prettier 兼容插件 */
const globals = require('globals');

module.exports = [
  {
    ignores: ['dist', 'out', '**/*.d.ts', 'node_modules', 'webpack.config.js', 'public'],
  },
  ...
  configPrettier,
  {
    ...
    rules: {
      ...
      camelcase: [
        'error',
        { properties: 'never', allow: ['__webpack_require__', '__non_webpack_require__'] },
      ], // 要求使用骆驼拼写法命名约定
