...
export class createTemplateConfigFile {
      ...
      vscode.commands.registerCommand(
        ...
        async () => {
          try {
            ...
            if (workspace_folder) {
              if (!this.haveConfig(workspace_folder)) {
                // this.copyFile();
  ...
  haveConfig(workspace_folder: vscode.WorkspaceFolder): Boolean {
    let workspace_configFileName = workspace_folder.uri.fsPath + '/.ftemplate.js';
    let workspace_configFileNameUrl = vscode.Uri.file(workspace_configFileName);
    if (fs.existsSync(workspace_configFileName)) {
      ...
      const workspace_template= this.getAllFile(path.normalize(path.resolve(workspace_folder.uri.fsPath, config_temp.templates.path)));
      const default_template= this.getAllFile(path.normalize(path.resolve(__dirname, '../../public/TemplateFileConfig/template')));
      vscode.window
        .showInformationMessage('配置文件已存在,是否要导入默认的模板文件？', '是', '否')
        .then((result) => {
          if (result === '是') {
            default_template.forEach(url => {
              if (!workspace_template.includes(url)) {
                this.copyFile(`${__dirname}/../../public/TemplateFileConfig/template${url}`,`${workspace_folder.uri.fsPath}/${config_temp.templates.path}${url}`);
              }
            });
            vscode.window.showInformationMessage('默认的模板文件完成导入!');
          } else {
      ...
      fs.readdir(filePath,  (err, files)=> {
        ...
        files.forEach((filename) => {
          ...
          fs.stat(filedir, (eror, stats) => {
            ...
            if (isFile) {
              fileList.push(filedir.replace(folderPath, '').replace(/\\/g, '/'));
            }
            // 如果是文件夹
            if (isDir) {
